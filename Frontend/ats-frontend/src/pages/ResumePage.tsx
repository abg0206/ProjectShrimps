...
export default function ResumePage() {
  ...
  // resume straight into the editor. Otherwise, if this job already has a
  // resume linked in the document library (S3-009/S3-010), load it so the
  // person is editing their real saved resume instead of a blank page.
  useEffect(() => {
    ...
    getJobDocuments(userEmail, tailoredJobId)
      .then(async (links) => {
        ...
        if (!incoming) {
          ...
          setSaveMessage(
            `Loaded your saved resume (v${link.version_number}) for this job.`
          );
        }
      })
      .catch((err) => console.error('Load linked resume error:', err))
  ...
  async function handleSave() {
    ...
    if (!tailoredJobId && !linkedDocumentId) {
      setSaveError(
        'Open a resume from a job or your Document Library first so it can be saved.'
      );
      return;
    }
    ...
    const title = tailoredFor ? `Resume for ${tailoredFor}` : 'Saved resume';

    try {
      if (linkedDocumentId) {
        // Already have a library document for this resume — content
        ...
        setSaveMessage(
          tailoredJobId ? 'Resume saved to this job.' : 'Resume saved.'
        );
        return;
      }
      ...
      if (linkResult.requiresConfirmation) {
        // S3-BR-011: a different resume is already linked to this job.
        // The new document stays in the library either way; ask before
        // replacing the link.
  ...
  return (
    ...
    >
      ...
      {/* Confirm-to-replace (S3-BR-011): a different resume is already
          linked to this job; the new one is saved to the library either
          way, this just confirms swapping the job's link. */}
      {pendingReplace && (
        <div
          style={{
            position: 'fixed',

...
export default function DashboardPage() {
  ...
  async function handleSaveEdit() {
    ...
    setSaving(true);
    try {
      const res = await fetch(`/api/jobs/${encodeURIComponent(userEmail)}/${editJob.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          stages: editStage,
  ...
  async function handleStatusChange(jobId: number, newStage: string) {
    try {
      const res = await fetch(`/api/jobs/${encodeURIComponent(userEmail)}/${jobId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const inputStyle = {
    ...
    border: 'none', fontSize: '14px', boxSizing: 'border-box' as const,
  };
  const labelStyle = {
    fontSize: '13px', color: '#3C1510', display: 'block', marginBottom: '4px',
  };
  const btnPrimary = (disabled = false) => ({
    backgroundColor: disabled ? '#c0847a' : '#932C20',
    color: '#E6CECB', padding: '8px 20px', borderRadius: '6px',
    border: 'none', cursor: disabled ? 'not-allowed' as const : 'pointer' as const, fontSize: '14px',
  });
  const btnSecondary = {
    backgroundColor: 'transparent', color: '#3C1510', padding: '8px 20px',
    borderRadius: '6px', border: '1px solid #3C1510', cursor: 'pointer' as const, fontSize: '14px',
  };
    ...
    <div style={{ display: 'flex', minHeight: '100vh', backgroundColor: '#D9958C' }}>
      ...
      <div style={{ flex: 1, padding: '32px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
          <h1 style={{ color: '#3C1510', fontSize: '24px', fontWeight: 'bold', margin: 0 }}>My Jobs</h1>
          <button onClick={() => { setShowAddModal(true); setModalError(''); }} style={btnPrimary()}>
            Add Job
          </button>
        </div>
        ...
        {loading ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px' }}>
            {filtered.map((job) => (
              <div key={job.id} style={{ backgroundColor: '#E6CECB', borderRadius: '10px', padding: '16px', display: 'flex', flexDirection: 'column', gap: '8px', minHeight: '200px' }}>
      ...
      {showAddModal && (
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }}>
          <div style={{ backgroundColor: '#E6CECB', borderRadius: '10px', padding: '24px', width: '400px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            {modalError && <p style={{ color: '#932C20', fontSize: '13px', margin: 0 }}>{modalError}</p>}
            <div>
              <label style={labelStyle}>Job Title</label>
              <input type="text" value={newTitle} onChange={(e) => setNewTitle(e.target.value)} style={inputStyle} />
            </div>
            <div>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px', marginTop: '8px' }}>
              ...
              <button onClick={handleAddJob} disabled={adding} style={btnPrimary(adding)}>
      ...
      {editJob && (
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }}>
          <div style={{ backgroundColor: '#E6CECB', borderRadius: '10px', padding: '24px', width: '400px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <h2 style={{ color: '#3C1510', fontSize: '18px', fontWeight: 'bold', margin: 0 }}>Edit Job</h2>
            {editError && <p style={{ color: '#932C20', fontSize: '13px', margin: 0 }}>{editError}</p>}
            <div>
              <label style={labelStyle}>Job Title</label>
            ...
            <div>
              <label style={labelStyle}>Company</label>
              <input type="text" value={editCompany} onChange={(e) => setEditCompany(e.target.value)} style={inputStyle} />
            </div>
            <div>
              <label style={labelStyle}>Job Description</label>
            ...
            <div>
              ...
              <select value={editStage} onChange={(e) => setEditStage(e.target.value)}
                style={{ ...inputStyle, border: '1px solid #932C20', backgroundColor: '#fff', cursor: 'pointer' }}>
                {Object.entries(STAGE_LABELS).map(([val, label]) => (
                  <option key={val} value={val}>{label}</option>
                ))}
              </select>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px', marginTop: '8px' }}>
              <button onClick={() => setEditJob(null)} style={btnSecondary}>Cancel</button>
